...
import {
  ...
  ChevronDown, ChevronUp, BookOpen, Newspaper
} from 'lucide-react';
...
export default function RSSReader({ type = 'news' }) {
  ...
  }, [type, pagination.page, pagination.limit, selectedSource, selectedKeyword, searchQuery, dateRange.start, dateRange.end]);

  // Reset page to 1 when filters change
  useEffect(() => {
    setPagination(prev => ({ ...prev, page: 1 }));
  }, [type, selectedSource, selectedKeyword, searchQuery, dateRange]);

  // Reset filters when type changes (navigating between News and Paper)
  useEffect(() => {
      resetFilters();
    ...
    <div className="flex h-screen bg-gray-100 text-slate-800 font-sans overflow-hidden">
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0 hidden md:flex">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          {/* Section Selector */}
          <div>
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">板块选择</h3>
              <div className="space-y-1">
                  <Link href="/news" className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors ${type === 'news' ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
                      <Newspaper className="w-4 h-4" />
                      新闻资讯
                  </Link>
                  <Link href="/paper" className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors ${type === 'paper' ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`}>
